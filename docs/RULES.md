...
6. **`npm run test` must pass** (Vitest). Pure logic lives in small modules
   (`scene/terrain.ts`, `utils/math.ts`, `player/circleAabbXZ.ts`, etc.) — add
   or extend tests there when behavior changes.
7. **`npm run smoke:ci` must pass** after any substantive change. It runs unit
...
8. **Rust `cargo test` in `server/`** must pass when `server/src/` or
   `server/Cargo.toml` changes.

## Module boundaries

9. **Scene modules expose interfaces, not meshes.** A scene builder returns
...
10. **Player/controls modules know nothing about meshes.** They only consume
    the scene's interface. This is what lets us swap levels.
11. **`Game.ts` is the only place that wires modules together.** It does not
    contain gameplay logic.
12. **No new files in `scene/` that mix geometry and collision rules.** Put
    pure terrain math in `scene/terrain.ts` (or a sibling), and the mesh-build
    in the scene file.

## Collision

13. The ground sampler is `scene/terrain.ts` → `sampleTerrainHeight`. Do not
    add a second source of truth (no per-mesh raycasts for ground).
14. Static obstacles are `AABBCollider`s. If a feature needs rotated or
...
15. The client predicts; the **server validates**. Inventory, mob HP, money,
...
16. Same code, both sides. Pure functions (terrain height, damage formulas,
    item math) live in modules importable by both client and server. No
    "client-only" gameplay shortcuts.

## Documentation

17. **Update `docs/TASKS.md`** when you finish a task or discover a new one.
    The task list is the project plan.
18. **Update `docs/ARCHITECTURE.md`** when you change a module boundary or
    the collision model.
19. Public interfaces get a doc comment that says *what it returns* and
    *what its limits are*. Limits are not optional — see the collision
    section in `ARCHITECTURE.md` for the format.

## Code style

20. Prefer small modules over big ones. If a file is over ~400 lines, split it.
21. No dead code, no commented-out blocks. `TODO(topic):` comments are fine
