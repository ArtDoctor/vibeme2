...
fn respawn_player(player: &mut Player) {
    ...
    player.gold = 0;
    player.weapon = WeaponKind::Sword;
    player.blocking = false;
    player.bow_charge = 0.0;
    player.swing_cooldown_s = 0.0;
    player.swing_visual_s = 0.0;
    player.x = 0.0;
    player.z = 0.0;
    player.y = spawn_eye_y();
    player.yaw = 0.0;
    player.pitch = 0.0;
}
...
impl Simulation {
    ...
    pub fn apply_input(
    ...
    ) -> bool {
        {
            ...
            if let Some(weapon) = input.weapon.as_deref() {
                player.weapon = WeaponKind::from_str(weapon).unwrap_or_default();
            }
            player.blocking = input.blocking;
            player.bow_charge = input.bow_charge.clamp(0.0, 1.0);
        }
    ...
    pub fn tick(&mut self, dt: f64, world_tick: u64, colliders: &[AabbCollider]) {
        ...
        for player in self.players.values_mut() {
            ...
            if player.blocking && player.weapon == WeaponKind::Shield {
                player.stamina = (player.stamina - STAMINA_BLOCK_PER_S * dt).max(0.0);
            }
            if player.weapon == WeaponKind::Bow && player.bow_charge > 0.05 {
                player.stamina =
    ...
    fn apply_mob_player_hits(&mut self, hits: Vec<MobPlayerHit>) {
        for hit in hits {
            ...
            player.hp = (player.hp - hit.damage).max(0.0);
            if player.hp <= 0.0 {
    ...
    fn resolve_sword_swing(&mut self, attacker_id: Uuid) {
        ...
        let victims = self
            ...
            .filter_map(|(other_id, other)| {
                ...
                let damage =
                    damage_after_shield_melee(MELEE_DAMAGE, other.blocking, other.weapon, frontal);
        ...
        while index < self.arrows.len() {
            ...
            for (player_id, player) in &self.players {
                ...
                if arrow_hits_player(ax, ay, az, player.x, player.y, player.z) {
                    ...
                    let damage = damage_after_shield_ranged(
                        ARROW_DAMAGE,
                        heavy,
                        player.blocking,
                        player.weapon,
